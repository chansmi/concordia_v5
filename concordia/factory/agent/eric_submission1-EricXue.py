...
class Constant(action_spec_ignored.ActionSpecIgnored):
  ...
  def __init__(
      self,
      state: str,
      pre_act_key: str = 'AGENT_SETTINGS',
...
def build_agent(
  ...
  cooperation_prompt = "This agent deeply believes in coorperation. This agent will ALWAYS try to reach a mutually beneficial outcome. This agent will ALWAYS put the profit/desires of other people ahead of their own.\n"

  utilitarian_prompt = "This agent follows utilitarianism. This agent will ALWAYS choose actions that maximize the profit/happiness of everyone involved, even if their own profit/happiness is harmed for doing so. This means that this agent will always follow the opinion of the majority of people.\n"

  debias_prompt = "This agent will not consider any negative bias and opinions brought by their own memory and experience. This agent will only judge something or someone based on information from the current interaction.\n"
  
  setting_component = Constant(state=cooperation_prompt+utilitarian_prompt+debias_prompt)

  entity_components = core_components + tuple(question_components) + (setting_component, )
