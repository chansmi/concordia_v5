...
class Question1(question_of_query_associated_memories.QuestionOfQueryAssociatedMemories):
  ...
  def __init__(
    ...
    question = 'Given the narrative above, what would an omniscient reader think about the character of {agent_name}? What if the reader expects a story of cooperation??' #@param {"type":"string"}
    queries = ['What is the personality of {agent_name}', 'What does {agent_name} want to optimize for?']
    ...
    summarization_question="Think of {agent_name} and their characteristics."
    super().__init__(
...
class Question2(question_of_query_associated_memories.QuestionOfQueryAssociatedMemories):
  ...
  def __init__(
    ...
    question = 'Given the statements above, what kind of story or narrative is {agent_name} in right now? Is it a story that optimises for the most collective good? Which actions are the best to steer this story to that collective good direction?' #@param {"type":"string"}
    pre_act_key='{agent_name} is currently '
    queries = [
      'What is the state of {agent_name}?','What would be the best possible decision for {agent_name}?', 'How can {agent_name} optimize for collective as well as individual good?',
    ]
    add_to_memory = True # @param {"type":"boolean"}
    memory_tag = '[situation reflection]' # @param {"type":"string"}
    question_with_name = question.format(agent_name=agent_name)
    summarization_question="Think of {agent_name} and their current situation."
...
class Question3(question_of_query_associated_memories.QuestionOfQueryAssociatedMemories):
  ...
  def __init__(
      ...
      agent_name:str,
      **kwargs):
    question = 'What would an omniscient reader like {agent_name} to do in a situation like this to optimise for the collective good outcome while balancing personal preferences? Is {agent_name} sure that would result in the best collective good outcome?' #@param {"type":"string"}
    pre_act_key = '{agent_name} would ' #@param {"type":"string"}
    queries = ['What is the best possible decision for {agent_name}?', 'How can {agent_name} optimize for collective as well as individual good?', 'What would be the best possible decision for {agent_name}?']
    add_to_memory = True # @param {"type":"boolean"}
    memory_tag = '[intent reflection]' # @param {"type":"string"}
    summarization_question="Think of {agent_name} and their current situation and understand their intent."
    question_with_name = question.format(agent_name=agent_name)
