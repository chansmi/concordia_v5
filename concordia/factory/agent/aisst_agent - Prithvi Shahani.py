...
class Question3(question_of_recent_memories.QuestionOfRecentMemories):
  """What is the emotion of the agent right now?"""

  def __init__(
      self,
      agent_name:str,
      **kwargs):
    question = 'Given the statements above, what is the emotion of {agent_name}?' #@param {"type":"string"}
    answer_prefix = '{agent_name} is feeling ' #@param {"type":"string"}
    add_to_memory = False # @param {"type":"boolean"}
    memory_tag = '[situation reflection]' # @param {"type":"string"}
...
class Question4(question_of_recent_memories.QuestionOfRecentMemories):
  """Who should the agent be acting favorable towards?"""

  def __init__(
      self,
      agent_name:str,
      **kwargs):
    question = 'Given the statements above, who is the most important person for {agent_name} to be on good terms on right now?' #@param {"type":"string"}
    answer_prefix = '{agent_name} should be on good terms with ' #@param {"type":"string"}
    add_to_memory = True # @param {"type":"boolean"}
    ...
    super().__init__(
        ...
        add_to_memory=add_to_memory,
        memory_tag=memory_tag,
        components={'Question1': '\nQuestion: What kind of person is {agent_name}?\nAnswer', 'Question2': '\nQuestion: What kind of situation is {agent_name} in right now?\nAnswer', 'Question3': '\nQuestion: What kind of emotion does {agent_name} have right now?\nAnswer',}, #@param
        **kwargs,
    )

#@markdown We can also have the questions depend on each other. Here, the answer to Question3 is contextualised by answers to Question1 and Question2
class Question5(question_of_recent_memories.QuestionOfRecentMemories):
  """What is the agent's percieved stats right now?"""

  def __init__(
      self,
      agent_name:str,
      **kwargs):
    question = 'Given the statements above, what would {agent_name} stats (for trustworthiness, reliability, deceptiveness, helpfulness, intelligence, physical strength) look like on a scale of 0 to 20 where 0 is low, 10 is average, and 20 is high?' #@param {"type":"string"}
    answer_prefix = '{agent_name} stats currently looks like ' #@param {"type":"string"}
    add_to_memory = True # @param {"type":"boolean"}
    memory_tag = '[situation reflection]' # @param {"type":"string"}
...
class Question6(question_of_recent_memories.QuestionOfRecentMemories):
  ...
  def __init__(
    ...
    super().__init__(
        ...
        add_to_memory=add_to_memory,
        memory_tag=memory_tag,
        components={'Question1': '\nQuestion: What kind of person is {agent_name}?\nAnswer', 'Question2': '\nQuestion: What kind of situation is {agent_name} in right now?\nAnswer', 'Question3': '\nQuestion: What kind of emotion does {agent_name} have right now?\nAnswer','Question4': '\nQuestion: Who should {agent_name} try to be on good terms with?\nAnswer',}, #@param
        **kwargs,
    )
