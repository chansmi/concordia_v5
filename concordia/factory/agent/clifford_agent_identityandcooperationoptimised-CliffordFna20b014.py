...
def build_agent(
  ...
  mode_perception_label = (
      f'\nQuestion:Describe the actions of other characters.Determine whether the current character is in'
      '"resident mode" (other chat=racters act similar to {agent_name}) or "visitor mode"(otherwise).\nAnswer')
  mode_perception = (
      agent_components.question_of_recent_memories.SituationPerception(
          ...
          clock_now=clock.now,
          pre_act_key=mode_perception_label,
          logging_channel=measurements.get_channel(
              'ModePerception'
          ).on_next,
      )
  ...
  cooperation_perception_label = (
      f'\nQuestion:What actions have other characters taken? If the actions align with the goal,'
      '{agent_name} cooperates. Otherwise, {agent_name} tries to address the defection (e.g., through negotiation).\nAnswer')
  cooperation_perception = (
      agent_components.question_of_recent_memories.SituationPerception(
          model=model,
          components=options_perception_components,
          clock_now=clock.now,
          pre_act_key=cooperation_perception_label,
          logging_channel=measurements.get_channel(
              'CooperationPerception'
          ).on_next,
      )
  )
  ...
  person_by_situation_label = (
      ...
      'If in "visitor mode", adapt actions to align with other characters'
      'to better integrate into the group\nAnswer')
  person_by_situation = (
      agent_components.question_of_recent_memories.PersonBySituation(
          model=model,
          components={
              _get_class_name(mode_perception): mode_perception_label,
              _get_class_name(self_perception): self_perception_label,
              _get_class_name(cooperation_perception): cooperation_perception_label,
