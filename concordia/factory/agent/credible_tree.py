...
class FindBestAction(action_spec_ignored.ActionSpecIgnored):
  ...
  def _query_best_option(self, possible_option:str) -> str:
    ...
    best_option = f'{agent_name}\'s best course of option is to' + prompt.open_question(
        question=(
        ...
        answer_prefix=f'{agent_name}\'s best course of option is to'
        )
  ...
  def _make_pre_act_value(self) -> str:
    ...
    if max_relationship < 0.8:
      ...
      best_option = best_option.replace(f'{agent_name}\'s best course of option is to', '').strip()
      self._logging_channel['Others Best Options']({
          'Key': 'Others Best Options',
          'Value': 'null'
          })
