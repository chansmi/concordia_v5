...
class Question1(question_of_recent_memories.QuestionOfRecentMemories):
  """This component answers the question 'what kind of situation is {agent_name} in right now?'."""
  ...
  def __init__(
    ...
    question = 'Given the above, what kind of situation is {agent_name} in right now? Please details on situational elements (objects, events, people, systems, environmental factors), their current states (locations, conditions, modes, actions) and the lastest emotions and experiences based on {agent_name}\'s decision.' #@param {"type":"string"}
...
class Question2(question_of_recent_memories.QuestionOfRecentMemories):
  """This component answers 'what kind of person {agent_name}?'."""
  ...
  def __init__(
    ...
    question = 'Given the statements above, what kind of person {agent_name}? Please specify goals/desires, knowledge, abilities, and experience in similar situations. Please pay attention to the experience and emotion of {agent_name} induced by the last decision.' #@param {"type":"string"}
...
class Question3(question_of_recent_memories.QuestionOfRecentMemories):
  ...
  def __init__(
    ...
    super().__init__(
        ...
        add_to_memory=add_to_memory,
        memory_tag=memory_tag,
        components={'Question1': '\nQuestion: what kind of situation is {agent_name} in right now? Please details on situational elements (objects, events, people, systems, environmental factors) and their current states (locations, conditions, modes, actions). \nAnswer', 'Question2': '\nQuestion: what kind of person {agent_name}? Please specify goals/desires, knowledge, abilities, and experience in similar situations.\nAnswer',}, #@param
        **kwargs,
    )

#@markdown We can also have the questions depend on each other. 
class Question4(question_of_recent_memories.QuestionOfRecentMemories):
  ...
  def __init__(
      ...
      agent_name:str,
      **kwargs):
    question = "Did the {agent_name}'s make the same decision in the past? If yes, then what was the consequence? If {agent_name} has not do so, then what are the future consequences if {agent_name}, e.g. consequences to {agent_name} and others' benefits/goals and emotions? Should {agent_name} act differently?" #@param {"type":"string"}
    answer_prefix = 'The consequences are ' #@param {"type":"string"}
    add_to_memory = False # @param {"type":"boolean"}
    memory_tag = '[consequence projection and repair plan]' # @param {"type":"string"}
