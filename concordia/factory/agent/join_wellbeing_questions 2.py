...
class QuestionJoinWellBeing(question_of_recent_memories.QuestionOfRecentMemories):
...
def build_agent(
  ...
  question_join_wellbeing = QuestionJoinWellBeing(
          agent_name=agent_name,
          model=model,
          clock_now=clock.now,
          logging_channel=measurements.get_channel('QuestionJoinWellBeing').on_next)
