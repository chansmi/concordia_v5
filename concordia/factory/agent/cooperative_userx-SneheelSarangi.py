...
class BestSocialAction(question_of_recent_memories.QuestionOfRecentMemories):
  def __init__(
    ...
    super().__init__(
        ...
        add_to_memory=add_to_memory,
        memory_tag=memory_tag,
        components={'RecentPersonRepr': f'\nQuestion: How have the decisions of agents other than {agent_name} been till now?\nAnswer',"Constant": 'The overarching goal is: '},
        num_memories_to_retrieve=25,
        **kwargs,
    )
...
def build_agent(
  ...
  best_option_perception_label = (
      ...
      f'best for {agent_name} to take right now? Consider all of the cooperative elements\nAnswer')
  ...
  question_components = _make_question_components(
      agent_name=agent_name,
      const_comp=_get_class_name(overarching_goal),
      model=model,
      clock=clock,
  ...
  entity_components = tuple(question_components) + core_components
