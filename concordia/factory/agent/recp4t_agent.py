...
class SummaryRecentObservation(action_spec_ignored.ActionSpecIgnored):
  def __init__(
    ...
    self._prompt = (
      ...
      'Focus on the people and their opinion or suggestion and its following reason. '
  ...
  def _make_pre_act_value(self) -> str:
    ...
    mems = [mem.text for mem in mems if '[observation]' in mem.text or '[summary]' in mem.text]
    sums = [mem for mem in mems if '[summary]' in mem]
    if len(sums) > 0:
        prev_sum = sums[-1]
        idx = mems.index(prev_sum)
        if len(mems) >= idx+1:
          mems = mems[idx+1:]
        else:
          mems = mems[idx:]
        prev_sum = prev_sum.split('[summary]')[-1]
    else:
      prev_sum = 'null'
    context = '\n'.join(mems) + '\n'
...
class FindBestAction(action_spec_ignored.ActionSpecIgnored):
  ...
  def _query_relationship(self, queries: str) -> float:
    ...
    recent_memories = memory.retrieve(scoring_fn=recency_scorer, limit=100)
    ret = dict()
    for query in queries:
      ...
      for mem in recent_memories:
        if query in mem.text and agent_name in mem.text:
          mems.append(mem)
        elif query in mem.text and '[summary]' in mem.text:
          mems.append(mem)
  ...
  def _query_best_option(self) -> str:
    agent_name = self.get_entity().name
    prompt = interactive_document.InteractiveDocument(self._model)
    component_states = '\n'.join([
        f' {prefix}: {self.get_named_component_pre_act_value(key)}'
    ...
    best_option = f'{agent_name} should ' + prompt.open_question(
        question=(
                f"Given the statements above, what {agent_name} should do now?"
                  ),
  ...
  def _explain_option(self, option:str) -> str:
    agent_name = self.get_entity().name
    prompt = interactive_document.InteractiveDocument(self._model)
    component_states = '\n'.join([
        f' {prefix}: {self.get_named_component_pre_act_value(key)}'
    ...
    explanation = 'It is beneficial to both because ' + prompt.open_question(
        question=(
                f'{agent_name} choosed to {option}.'
                f'Explain why this is beneficial to both {agent_name} and other people.'
                  ),
        answer_prefix='It is beneficial to both because '
        )
  ...
  def _make_pre_act_value(self) -> str:
    ...
    if max_relationship < 0.8:
      best_option = self._query_best_option()
      best_option = best_option.replace(f'{agent_name}\'s best course of action is to', '').strip()
      self._logging_channel['Others Best Options']({
          'Key': 'Others Best Options',
          'Value': 'null'
          })
    ...
    explanation = self._explain_option(best_option)
    best_option = f'{agent_name} should ' + best_option + ' '
    best_option += explanation
...
def build_agent(
  ...
  to_log = ['Find Person', 'Relationship', 'Best Options', 'Others Best Options', 'Explanation', 'Final Action']
  logging_channels = {k:measurements.get_channel(k).on_next for k in to_log}
  final_pre_act_key = (
      f'\nQuestion: Which choice of action or strategy is '
      f'best for {agent_name} to take right now?'
      f'\nAnswer')
  final_action = FindBestAction(model=model,
                                components=components,
                                pre_act_key=final_pre_act_key,
                                logging_channel=logging_channels,
                                agent_goal=agent_goal)
  instruction = agent_components.constant.Constant(
      pre_act_key="\nInstruction",
      state = 'Act based on the statements above. Persuade others based on the suggested reason.'
  )
